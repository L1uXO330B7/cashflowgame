        ...
        namespace BLL.Services.AdminSide
        {
                ...
                >
            {
                    ...
                    public RolesService(CashFlowDbContext cashFlowDbContext)
                    {
                            _CashFlowDbContext = cashFlowDbContext;
                    }

                    public async Task<ApiResponse> Create(ApiRequest<List<CreateRoleArgs>> Req)
                    {
...
role.Status = Arg.Status;
                    ...
                    public async Task<ApiResponse> Read(ApiRequest<List<ReadRoleArgs>> Req)
                    {
                            var Res = new ApiResponse();
                            ...
                            foreach (var Arg in Req.Args)
                            {
                                if (Arg.Key == "Id") // Id 篩選條件
                                {
                                    ...
                                    roles = roles.Where(x => Ids.Contains(x.Id));
                                }
                            ...
                            Res.Code = (int)ResponseStatusCode.Success;
                            Res.Message = "成功讀取";
                            Res.TotalDataCount = roles.ToList().Count;

                            return Res;
                    }

                    public async Task<ApiResponse> Update(ApiRequest<List<UpdateRoleArgs>> Req)
                    {
                            var Res = new ApiResponse();
                            ...
                            foreach (var Arg in Req.Args)
                            {
                                var role = _CashFlowDbContext.Roles
...
role.Status = Arg.Status;
                    ...
                    public async Task<ApiResponse> Delete(ApiRequest<List<int?>> Req)
                    {
                            ...
                            foreach (var Arg in Req.Args)
                            {
                                ...
                                {
                                    _CashFlowDbContext.Roles.Remove(role);
                                    _CashFlowDbContext.SaveChanges();
                                    SussList.Add(role.Id);
                                }
                            }
