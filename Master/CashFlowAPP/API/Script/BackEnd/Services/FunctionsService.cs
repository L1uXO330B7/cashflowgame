        ...
        namespace BLL.Services.AdminSide
        {
            public class FunctionsService : IFunctionsService<
                    List<CreateFunctionArgs>,
                    List<ReadFunctionArgs>,
                    List<UpdateFunctionArgs>,
                    List<int?>
                >
            {
                    private readonly CashFlowDbContext _CashFlowDbContext;

                    public FunctionsService(CashFlowDbContext cashFlowDbContext)
                    {
                            _CashFlowDbContext = cashFlowDbContext;
                    }

                    public async Task<ApiResponse> Create(ApiRequest<List<CreateFunctionArgs>> Req)
                    {
                            ...
                            foreach (var Arg in Req.Args)
                            {
                                ...
                                function.Id = Arg.Id;
function.Description = Arg.Description;
function.Status = Arg.Status;


                                functions.Add(function);
                            }

                            _CashFlowDbContext.AddRange(functions);
                            _CashFlowDbContext.SaveChanges();
                            // 不做銷毀 Dispose 動作，交給 DI 容器處理

                            // 此處 SaveChanges 後 SQL Server 會 Tracking 回傳新增後的 Id
                            SussList = functions.Select(x => x.Id).ToList();
                    ...
                    public async Task<ApiResponse> Read(ApiRequest<List<ReadFunctionArgs>> Req)
                    {
                            var Res = new ApiResponse();
                            var functions = _CashFlowDbContext.Functions.AsQueryable();
                            ...
                            foreach (var Arg in Req.Args)
                            {
                                if (Arg.Key == "Id") // Id 篩選條件
                                {
                                    ...
                                    functions = functions.Where(x => Ids.Contains(x.Id));
                                }
                                ...
                                if (Arg.Key == "Status") // 狀態篩選條件
                                {
                                    ...
                                    functions = functions.Where(x => x.Status == Status);
                                }
                            }
                            ...
                            Res.Code = (int)ResponseStatusCode.Success;
                            Res.Message = "成功讀取";
                            Res.TotalDataCount = functions.ToList().Count;

                            return Res;
                    }

                    public async Task<ApiResponse> Update(ApiRequest<List<UpdateFunctionArgs>> Req)
                    {
                            var Res = new ApiResponse();
                            ...
                            foreach (var Arg in Req.Args)
                            {
                                var function = _CashFlowDbContext.Functions
...
function.Status = Arg.Status;
                    ...
                    public async Task<ApiResponse> Delete(ApiRequest<List<int?>> Req)
                    {
                            ...
                            foreach (var Arg in Req.Args)
                            {
                                ...
                                {
                                    _CashFlowDbContext.Functions.Remove(function);
                                    _CashFlowDbContext.SaveChanges();
                                    SussList.Add(function.Id);
                                }
                            }
